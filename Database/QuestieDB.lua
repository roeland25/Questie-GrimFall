...
function QuestieDB.GetQuestIDFromName(name, questgiverGUID, questStarter)
    QuestieDB.GrimfallMissingQuests = QuestieDB.GrimfallMissingQuests or {}

    local questID = 0 -- worst case, return 0 if no match is found
    ...
    if questgiverGUID then
        ...
        elseif unit_type == "GameObject" then
        ...
        if questStarter == true then
            ...
            if questsStarted then
                for _, id in pairs(questsStarted) do
                    if name == QuestieDB.QueryQuestSingle(id, "name") then
                        questID = id
                        break
                    end
                end
            ...
            ------------------------------------------------------------
            -- Grimfall dungeon quest pet
            --
            -- Creature 2353 can offer normal dungeon quests even though
            -- it is not listed as their original Blizzard quest starter.
            --
            -- If the normal starter lookup failed, search Questie's
            -- existing quest database for an exact quest-name match.
            ------------------------------------------------------------
            if questID == 0 and unit_type == "Creature" and questgiverID == 2353 then

                for id, _ in pairs(QuestieDB.QuestPointers) do
                    local questName = QuestieDB.QueryQuestSingle(id, "name")
            ...
            ------------------------------------------------------------
            -- Still unknown = genuinely missing/custom Grimfall quest
            ------------------------------------------------------------
            if questID == 0 and Questie.IsSoD == false then
                QuestieDB.GrimfallMissingQuests[name] = {
                    name = name,
                    giverType = unit_type,
        ...
        ------------------------------------------------------------
        -- QUEST ENDER / ACTIVE QUEST
        ------------------------------------------------------------
        else

            -- First try the normal Questie questEnds database.
            if questsEnded then
                for _, id in pairs(questsEnded) do
                    if (name == QuestieDB.QueryQuestSingle(id, "name"))
                    and QuestiePlayer.currentQuestlog[id]
                then
                    questID = id
                    break
                end
            ...
            -- Grimfall fallback
            --
            -- A custom quest may appear as "active" on a gossip frame
            -- even when this NPC/object is NOT actually its quest ender.
            --
            -- If the quest is already in our Grimfall override database
            -- and is currently in the player's quest log, identify it by
            -- name without adding this object as an ender.
            ------------------------------------------------------------
            if questID == 0 then
                for id, _ in pairs(QuestiePlayer.currentQuestlog) do

                    if QuestieDB.questDataOverrides[id]
                        and name == QuestieDB.QueryQuestSingle(id, "name")
                    then
                        questID = id
                        break
                    end
            ...
            ------------------------------------------------------------
            -- Only report a genuinely unknown active/ender quest.
            ------------------------------------------------------------
            if questID == 0 and Questie.IsSoD == false then
                QuestieDB.GrimfallMissingQuests[name] = {
                    name = name,
                ...
                Questie:Print(
                    "|cffffd200[Grimfall]|r Missing ender: "
                    .. name
                    .. " from "
                    .. unit_type
                    .. " "
